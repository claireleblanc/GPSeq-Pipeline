...
libid=$1
input="/home/fastq/${libid}.fastq.gz"
cut_enzyme="MboI"
cut_seq="GATC"

ref_genome="Mus_musculus.GRCm38"
ref_dir="/home/ref"
bowtie2_ref="${ref_dir}/${ref_genome}.95.dna.primary_assembly"
...
then
    wget -O ${ref_dir}/${ref_genome}.95.dna.primary_assembly.noChr.fa.gz http://ftp.ensembl.org/pub/release-95/fasta/mus_musculus/dna/${ref_genome}.dna.primary_assembly.fa.gz
    zcat ${ref_dir}/${ref_genome}.95.dna.primary_assembly.noChr.fa.gz | sed 's/>/>chr/' | cut -d" " -f 1 | gzip > ${ref_dir}/${ref_genome}.95.dna.primary_assembly.fa.gz
    rm ${ref_dir}/${ref_genome}.95.dna.primary_assembly.noChr.fa.gz
    /app/bin/bowtie2-build ${ref_dir}/${ref_genome}.95.dna.primary_assembly.fa.gz ${ref_dir}/${ref_genome}.95.dna.primary_assembly --threads 4 --verbose
fi

if [[ ! -s "${cutsite_path}" ]]
then
    fbarber find_seq ${ref_dir}/${ref_genome}.95.dna.primary_assembly.fa.gz ${cut_seq} --case-insensitive --global-name --output ${ref_dir}/${ref_genome}.95.dna.primary_assembly.${cut_enzyme}_sites.bed.gz --log-file ${ref_dir}/${ref_genome}.95.dna.primary_assembly.${cut_enzyme}_sites.log
fi
