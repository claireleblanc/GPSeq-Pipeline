...
for library_iid in range(len(library_id_list)):
    ...
    log_path = os.path.join(args.root, "output/fastq_hq", f"{library_id}.log")
...
for library_iid in range(len(library_id_list)):
    library_id = library_id_list[library_iid]
    log_path = os.path.join(args.root, "output/fastq_prefix", f"{library_id}.log")
    assert os.path.isfile(log_path)
    with open(log_path) as LH:
        matched = False
        for line in LH:
...
for library_iid in range(len(library_id_list)):
    library_id = library_id_list[library_iid]
    log_path = os.path.join(args.root, "output/mapping", f"{library_id}.mapping.log")
    assert os.path.isfile(log_path)
    with open(log_path) as LH:
        matched = False
        for line in LH:
...
for library_iid in range(len(library_id_list)):
    library_id = library_id_list[library_iid]
    log_path = os.path.join(args.root, "output/mapping", f"{library_id}.chrM.txt")
    assert os.path.isfile(log_path)
    with open(log_path) as LH:
...
for library_iid in range(len(library_id_list)):
    library_id = library_id_list[library_iid]
    log_path = os.path.join(args.root, "output/mapping", f"{library_id}.lq_count.txt")
    assert os.path.isfile(log_path)
    with open(log_path) as LH:
...
for library_iid in range(len(library_id_list)):
    library_id = library_id_list[library_iid]
    log_path = os.path.join(args.root, "output/mapping", f"{library_id}.clean_count.txt")
    assert os.path.isfile(log_path)
    with open(log_path) as LH:
...
for library_iid in range(len(library_id_list)):
    library_id = library_id_list[library_iid]
    log_path = os.path.join(args.root, "output/atcs", f"{library_id}.clean.umis_at_cs.txt.log")
...
for library_iid in range(len(library_id_list)):
    ...
    log_path = os.path.join(
        args.root, "output/dedup", f"{library_id}.clean.umis_at_cs.txt.gz.umi_prep_notes.txt"
    )
    assert os.path.isfile(log_path), f"file not found: '{log_path}'"
    with open(log_path) as LH:
        matched = False
...
dataframe.sort_values("library_id").to_csv(
    os.path.join(args.root, "output/summary_table.tsv"), sep="\t", index=False
)
