...
patterns[
    "quality_filters"
] = ".* ([0-9]+)/([0-9]+) \(([0-9\.%]+)\).*flag_extract\.py:246" #changed from 251
patterns[
    "prefix"
] = ".* ([0-9]+)/([0-9]+) \(([0-9\.%]+)\).*flag_regex\.py:156" #changed from 161
...
fastq_list = glob(os.path.join(fastq_dir_path, "*")) #['/Users/claireleblanc/gpseq-tutorial/fastq/TUTORIAL01_S1_LALL_R1_001.fastq.gz']
library_id_list = [os.path.basename(x).split("_")[0] for x in fastq_list]
#print(library_id_list) ['TUTORIAL01']
...
for library_iid in range(len(library_id_list)):
    ...
    #print(log_path) Users/claireleblanc/gpseq-tutorial/fastq_hq/TUTORIAL01.log
    ...
    with open(log_path) as LH:
        ...
        #wasn't working, had to change regx to be correct line
...
for library_iid in range(len(library_id_list)):
    ...
    assert os.path.isfile(log_path)
    #print(log_path) /Users/claireleblanc/gpseq-tutorial/fastq_prefix/TUTORIAL01.log
    with open(log_path) as LH:
        matched = False
        for line in LH:
